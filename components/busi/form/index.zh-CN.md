...
表单布局与操作栏组件族：`FormRow`（纯 CSS 等分布局）、`FormARow`（基于栅格的布局，支持跨列）、`FormACol`（跨列表单项容器）、`FormBox`（白色容器）、`FormTitle`（标题栏）、`FormFooter`（底部操作栏）。基于 Flex 或 24 栅格实现多列等分布局，子项自动过滤空节点。
...
- 当需要区块标题与右侧操作区时，使用 `FormTitle`。
- 当需要统一的底部操作栏、或需要操作栏常驻底部时，使用 `FormFooter`。
...
区块标题栏，左侧为标题文案、右侧为操作区，中间以分割线填充。

| 参数 | 说明 | 类型 | 默认值 | 版本 |
| --- | --- | --- | --- | --- |
| inset | 左右内缩：数字补 px，带单位字符串原样透传，`0` 表示与宿主容器对齐 | string \| number | `20` |  |
...
底部操作栏，默认随内容滚动；`fixed` 时常驻底部。

| 参数 | 说明 | 类型 | 默认值 | 版本 |
| --- | --- | --- | --- | --- |
| fixed | 常驻底部（`position: absolute`，定位锚点由父容器提供，组件不创建） | boolean | `false` |  |
| inset | 左右内缩：数字补 px，带单位字符串原样透传，`0` 即通栏 | string \| number | `12` |  |

#### FormFooter 插槽

| 插槽名称 | 说明                                               | 版本 |
| -------- | -------------------------------------------------- | ---- |
| default  | 操作区内容，通常为 `RadiusButton`，彼此左间距 16px |      |
